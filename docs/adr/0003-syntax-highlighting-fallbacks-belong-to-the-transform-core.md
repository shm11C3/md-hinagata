...
- The code block template context keeps `lang`, `raw`, and `code` for compatibility. `code` is source code text intended for normal escaped Template Interpolation insertion with `{{code}}`; it is not pre-escaped HTML. `raw` remains for compatibility but is deprecated for new examples and must not be inserted with triple braces.
